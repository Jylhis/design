...
Token sources: [`tokens.core.json`](./tokens.core.json) +
[`themes/jylhis.json`](./themes/jylhis.json). Consumer guide:
[`docs/INTEGRATION.md`](./docs/INTEGRATION.md); design contract:
[`DESIGN.md`](./DESIGN.md).

## [Unreleased]

### Removed
- Duplicated documentation; code and the token sources are the documentation.
  `tokens.md` (a third copy of the token values), `PRODUCT.md` (stale
  Sheet/Field world), `ENGINEERING_PRINCIPLES.md` + `WAY_OF_WORKING.md`
  (duplicated AGENTS.md), `docs/PRINCIPLES.md` + `docs/VOICE.md` +
  `docs/THEMING.md` (duplicated DESIGN.md / AGENTS.md), the historical audit
  records (`docs/REVIEW.md`, `docs/VALIDATION.md`,
  `docs/design-review-response.md`, `docs/v2/`), and the stale generated
  `docs/components/` snapshot. DESIGN.md's YAML frontmatter (a full hex copy
  of the theme) is gone; README's v1-remnant VISUAL FOUNDATIONS /
  ICONOGRAPHY / CONTENT FUNDAMENTALS sections are gone. Every pointer that
  named a deleted file now names the token sources or DESIGN.md.

### Added
- 7-step contrast-solved ramps for every chromatic role (`accent`, `destructive`, `brand`, `contour`, `status-*`) in both themes, emitted as `--color-<role>-1..7`.
- `--color-destructive`: the irreversible-action role, always the higher-contrast of the destructive/accent pair. Registered as the `consequence` group.
- `--shadow-float`: the one elevation shadow, for objects that leave the flow (modal, sheet, popover, toast, dropdown), stated per mode.
- `density.css` + the `data-density` axis (comfortable 48 / default 40 / compact 32px; coarse-pointer floor 40px). Density box tokens (`--cell-h`, `--control-h`, `--touch-min`) emitted in `tokens.css` and remapped per scope.
- Fourth font role: `--font-numeric` (IBM Plex Sans Condensed 400/500/600) for amounts, readouts, tabular figure columns.
- Radii `lg 8px` and `xl 12px`; `md` retuned 4px → 6px.
- Ten components: Cell, CellGroup, Descriptions, Empty, EntryGrid, Result, Segmented, SkillMatrix, Steps, Toast. CvEntry and Field reworked.

### Changed
- Survey and Mono palettes retuned to the 2026-09 seed-solved values (accent `#693900`/`#f5a351`, grounds `#f5f8fc`/`#0c0f14`, ink ladder and Modus syntax re-solved against the new grounds).
- `--color-accent-subtle` is now a solid ramp-step-1 tint instead of an alpha composite.
- Mono status colors grey into a severity ladder of lightness (glyph + word carry the meaning); ANSI slots stay chromatic for the TTY.
- Platform `_reference` survey files retuned so derived targets recolor with the palette.

### Fixed

- Documentation retargeted to the 3.0.0 single-theme outputs: platform
  READMEs (gtk, rofi, ghostty, hyprland, gimp, adobe, waybar, mako) name the
  `jylhis-light` / `jylhis-dark` files, `docs/INTEGRATION.md` shows the real
  `mkPalette { mode = ...; }` / `toBase16Scheme` signatures and the four font
  families (incl. `density.css` and `--font-numeric`), the emacs `_reference`
  headers cite `tokens.core.json` + `themes/jylhis.json`, and specimen copy no
  longer references the retired `tokens.json`.


## [3.0.0] — 2026-09-22

The theming framework collapses to a **single theme**: Survey and Monochrome
merge into one `jylhis` theme whose values are the Survey palette and whose
structure is the monochrome skeleton (ramps, density axis, syntax emphasis
weights). Design rationale lives in [`docs/THEMING.md`](./docs/THEMING.md).

### Changed — BREAKING

- **One theme, two modes.** `themes/survey.json` and `themes/mono.json` are
  replaced by `themes/jylhis.json` (Survey values; mode labels Print/Negative).
  `tokens.core.json#meta.themes` registers the single slug `jylhis`.
- **`data-theme` is retired.** CSS scopes are `:root` (light) and
  `[data-mode="dark"]` only. The showcase `#theme-select` control, the
  `jylhis-ds-theme` localStorage key, and the Tweaks panel Theme radio are
  removed; the mode key `jylhis-theme` stays. The mono-theme specimen card
  (`preview/colors-mono.html`) is deleted; the grayscale check lives on as the
  Tweaks grayscale toggle.
- **Generated outputs are renamed.** Every platform file moves from
  `jylhis-<theme>-<light|dark>` (×4 variants) to `jylhis-<light|dark>` (×2):
  `platforms/{ghostty,rofi,hyprland,gimp,base16,console,emacs,bat,shell}`,
  kvantum `JylhisLight.colors`/`JylhisDark.colors`, mako `config-<m>`, waybar
  `style-<m>.css`, gtk `jylhis-<m>.css` (light also copied to `gtk.css`),
  shadcn `jylhis-light.css`/`jylhis-dark.css`. `waybar/style.css` and
  `mako/config` remain dark copies for name-hardcoding consumers.
- **`tokens-data.js` is flat.** The top level IS the theme; the per-theme
  `themes` sub-object is gone.

### Removed

- The `[data-theme="mono"]` component overrides in `components/components.css`
  (chevron, StatusBadge) — the base rules carry the single theme.

### Upgrade notes

- Consumers pinning `jylhis-survey-*` / `jylhis-mono-*` file names must move
  to `jylhis-light` / `jylhis-dark`. Consumers of `tokens-data.js#themes` read
  the top level instead. Generated files are scheduled to stop being tracked
  in git; build them with `bun scripts/generate.mjs`.
- External consumers that hardcode the old slugs (marchyo's vicinae/bat
  modules) need a follow-up rename in their own repo.
