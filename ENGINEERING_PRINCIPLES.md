...
One brand color, two roles: `--color-brand` for the maker's-mark chevron
and the datum mark, `--color-accent` for any accent that carries text
meaning (links, focus rings, primary fills). Status is always the Modus
red/yellow/green/cyan signal tokens, never the accent. The bronze accent
is **deliberately not a syntax color**, and `contour` blue is structure
only — it never stands in for interaction.
...
emoji. The maker's mark is pure type — the prompt `jy ❯` — and the only
bespoke SVG in the system is `assets/favicon.svg`, which renders it.
...
The generation check plus six validators run on every push and PR:
...
- `bun scripts/validate-emacs-faces.mjs` — Emacs face coverage matches
  `face-manifest.json`.
- `bun scripts/validate-preview-hex.mjs` — hex literals in showcase HTML
  exist in `tokens.json`.
...
triad, a fixed list of platform targets. Adding a platform target or a
