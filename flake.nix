{
  ...
  outputs =
    ...
    in
    {
      ...
      lib = {
        # Absolute store path to the generated base16 YAML for a mode
        # ("light" | "dark"), suitable for `stylix.base16Scheme`. Requires
        # the overlay (or a pkgs set that already has `jylhis-themes`).
        toBase16Scheme = pkgs: mode: "${pkgs.jylhis-themes}/share/jylhis/base16/jylhis-${mode}.yaml";

        # Reads the token sources + generated base16 YAML and returns the
        # palette in the shapes downstream Nix configs need:
        # { hex; ansi; ansi16; base16; }. No pkgs needed.
        mkPalette =
          { mode }:
          import ./nix/palette.nix {
            inherit (nixpkgs) lib;
            inherit mode;
          };
      };
