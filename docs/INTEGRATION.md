...
Grotesk, and IBM Plex Mono live in `fonts.css`, and the `woff2` files ship
in `fonts/`. Vendor both next to `tokens.css` — `colors_and_type.css`
imports `fonts.css` internally, so no font CDN is involved. If your static root differs from the CSS
...
Field is emitted under both `.dark` and `[data-theme="dark"]`, so it works
...
ANSI 11 is always the bronze accent (`#6f3e00` light, `#e0a33a` dark).
...
| ------------ | --------------- | ----------------------------------------- |
| `border`     | `bright-black`  | faint — `#878c95` · `#656b76`             |
| `text`       | `white`         | body text — `#23262e` · `#d6dae2`         |
| `time`       | `cyan`          | syn-type — Modus cyan-cooler              |
| `container`  | `black`         | bg — `#f6f8fb` · `#0d0f14`                |
| `prompt`     | `bright-yellow` | **bronze accent** — `#6f3e00` · `#e0a33a` |
| `input`      | `white`         | body text (foreground)                    |
| `action`     | `bright-yellow` | bronze accent                             |
| `button`     | `magenta`       | Modus magenta                             |
| `greet`      | `bright-white`  | text-heading — `#12141a` · `#f2f4f8`      |
...
```json
{
  "mcpServers": {
    "jylhis-design": {
      "command": "sh",
      "args": ["-c", "cd platforms/mcp && exec go run ."]
    }
  }
}
```
