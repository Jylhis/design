...
// ─── Theme editions ──────────────────────────────────────────────────
// Sheet (light) and Field (dark) are the colour editions; Chalk (mono-light)
// and Graphite (mono-dark) are the monochrome pair. Every generator resolves a
// mode's identity from this table instead of branching on the mode string, so a
// new edition is a data addition here rather than a new branch in each function.
//   key       — the token key resolved by color(role, key)
//   label     — human theme name (headers, doc columns)
//   file      — filename / theme-name token (jylhis-<file>)
//   kv        — Kvantum theme attribute name
//   polarity  — "light" | "dark"; drives light-vs-dark structural choices
//   modus     — the Emacs Modus edition a colour theme mirrors, or null for mono
//   dataTheme — CSS selector carrying the theme (":root" for the default light)
//   scrimAlpha — modal/overlay scrim opacity for this polarity
//   sibling   — the theme the 2-way toggle flips to (stays within an edition)
const MODES = {
  "light": {
    key: "light", label: "Sheet", file: "sheet", kv: "JylhisSheet",
    polarity: "light", modus: "Operandi", dataTheme: ":root",
    scrimAlpha: "0.4", sibling: "dark",
  },
  "dark": {
    key: "dark", label: "Field", file: "field", kv: "JylhisField",
    polarity: "dark", modus: "Vivendi", dataTheme: '[data-theme="dark"]',
    scrimAlpha: "0.55", sibling: "light",
  },
  "mono-light": {
    key: "mono-light", label: "Chalk", file: "chalk", kv: "JylhisChalk",
    polarity: "light", modus: null, dataTheme: '[data-theme="mono-light"]',
    scrimAlpha: "0.4", sibling: "mono-dark",
  },
  "mono-dark": {
    key: "mono-dark", label: "Graphite", file: "graphite", kv: "JylhisGraphite",
    polarity: "dark", modus: null, dataTheme: '[data-theme="mono-dark"]',
    scrimAlpha: "0.55", sibling: "mono-light",
  },
};
const ALL_MODES = Object.keys(MODES);
const COLOR_MODES = ["light", "dark"]; // the two Modus-backed colour editions
const isLight = (mode) => MODES[mode].polarity === "light";
...
// ─── Helper: hex → OKLCH string ("oklch(L% C H)") ────────────────────
// tokens.css expresses every colour in OKLCH: a perceptual space where the
// system's contrast/lightness relationships are legible in the value itself
// ("contrast is plotted, not asserted"). tokens.json stays the hex datum — the
// contrast validators read the hex, so the guarantee is unchanged; this is a
// lossless re-encoding of the same in-gamut sRGB colour. Precision is high
// enough to round-trip each hex to itself, and fixed so the generated file is
// byte-stable (no float noise). Achromatic greys collapse to chroma/hue 0.
...
function hexToOklch(hex) {
  ...
  if (h < 0) h += 360;
  const achromatic = C < 5e-4;
  const Lp = (L * 100).toFixed(4);
  const Cs = (achromatic ? 0 : C).toFixed(5);
  const hs = (achromatic ? 0 : h).toFixed(3);
  return `oklch(${Lp}% ${Cs} ${hs})`;
}

// ─── Helper: hex → bare HSL triplet ("H S% L%") ──────────────────────
// Tailwind/shadcn consumers wrap these as hsl(var(--x) / <alpha-value>), so the
// channels must be emitted without the hsl() wrapper for opacity modifiers to
// work. Rounded to one decimal — enough to round-trip every token to its exact
// byte, and stable across platforms (no float noise in the generated file).
...
// ─── Helper: flatten a colour over a background at a given alpha ──────
// Mirrors blendHex() in validate-tokens.mjs so the two never disagree.
...
function generateTokensCSS() {
  ...
  // Colours are emitted in OKLCH (lossless re-encoding of the hex datum); the
  // per-edition [data-theme] structure is unchanged.
  const ok = (hex) => hexToOklch(hex);
  const varBlock = (mode) => {
    const lines = [];
    // Group: palette
    lines.push("  /* Background */");
    for (const r of ["bg", "bg-subtle"]) lines.push(`  ${cssName(r)}: ${ok(tokens.palette[r][mode])};`);
    lines.push("  /* Surface */");
    for (const r of ["surface", "surface-raised"]) lines.push(`  ${cssName(r)}: ${ok(tokens.palette[r][mode])};`);
    lines.push("  /* Text */");
    for (const r of ["text", "text-muted", "text-heading", "text-faint"]) lines.push(`  ${cssName(r)}: ${ok(tokens.palette[r][mode])};`);
    lines.push("  /* Accent */");
    for (const r of ["accent", "accent-hover"]) lines.push(`  ${cssName(r)}: ${ok(tokens.palette[r][mode])};`);
    // accent-subtle is derived (rgba), keep as hand-authored in colors_and_type.css
    lines.push(`  ${cssName("brand")}: ${ok(tokens.palette.brand[mode])};`);
    // selection highlight + input caret (matches the editor/terminal targets)
    for (const r of ["selection-bg", "cursor"]) lines.push(`  ${cssName(r)}: ${ok(tokens.palette[r][mode])};`);
    lines.push("  /* Borders */");
    for (const r of ["border", "border-strong", "decorator", "contour"]) lines.push(`  ${cssName(r)}: ${ok(tokens.palette[r][mode])};`);
    lines.push("  /* Code */");
    lines.push(`  --color-code-bg: ${ok(tokens.palette["bg-subtle"][mode])};`);
    lines.push(`  --color-code-text: ${ok(tokens.palette.text[mode])};`);
    lines.push(`  --color-code-border: ${ok(tokens.palette.border[mode])};`);
    lines.push("  /* Syntax */");
    for (const r of Object.keys(tokens.syntax)) lines.push(`  ${cssName(r)}: ${ok(tokens.syntax[r][mode])};`);
    lines.push(`  --color-syntax-tag: ${ok(tokens.syntax["syn-type"][mode])};`);
    lines.push("  /* Status */");
    for (const r of Object.keys(tokens.status)) lines.push(`  ${cssName(r)}: ${ok(tokens.status[r][mode])};`);
    ...
    for (const [surface, spec] of Object.entries(tokens.pairs ?? {})) {
      lines.push(`  ${cssName(surface)}-foreground: ${ok(allRoles[spec.fg][mode])};`);
    }
    return lines.join("\n");
  };
  ...
  // Type scaling floors — the readable/chrome boundary from
  // typography.scaling. Hand CSS uses --type-floor to floor relative (em)
  // sizes so nesting cannot compound below the hard minimum, and
  // --type-readable-min documents the step below which text a user must read
  // may not sit. Spec: docs/ACCESSIBILITY.md § Text resizing & reflow.
  const scalingVars = [
    `  --type-readable-min: ${tokens.typography.scaling.readableFloor};`,
    `  --type-floor: ${tokens.typography.scaling.absoluteFloor};`,
  ].join("\n");
  ...
  // accent-subtle sits a touch stronger on the dark polarities so the tint reads
  // against the darker ground; scrim opacity comes from the mode descriptor.
  const derivedVars = (mode) => {
    const subtleAlpha = isLight(mode) ? 0.12 : 0.15;
    return [
      `  --color-accent-subtle: ${rgbaOf("accent", mode, subtleAlpha)};`,
      `  --color-scrim: ${rgbaOf("scrim", mode, MODES[mode].scrimAlpha)};`,
    ].join("\n");
  };
  // Extra theme editions beyond the :root default, each under its own selector.
  const extraThemeBlocks = ALL_MODES.filter((m) => m !== "light")
    .map((mode) => `${MODES[mode].dataTheme} {\n${varBlock(mode)}\n${derivedVars(mode)}\n}`)
    .join("\n\n");
...
/* Dark (Field), and the monochrome Chalk / Graphite editions */
${extraThemeBlocks}
`;
}

// ─── 1b. platforms/shadcn/tokens.css — Tailwind / shadcn bridge ──────
//
// Tailwind consumers cannot use the hex vars in tokens.css: `bg-primary/10`
// compiles to `hsl(var(--primary) / 0.1)`, which needs bare HSL channels. This
// target emits the same palette under the shadcn semantic names, as triplets,
// so a Tailwind app gets v2 without hand-converting anything.
//
// The file is deliberately SELF-CONTAINED — a consumer imports this one
// stylesheet (plus fonts.css) and nothing else. That means it restates the font
// families and radius rather than importing tokens.css, so the two must both be
// generated from tokens.json; neither is hand-maintained.
//
// Field is emitted under BOTH `.dark` (Tailwind's `darkMode: ["class"]`
// default) and `[data-theme="dark"]` (this system's own convention elsewhere),
// so the file drops into either setup unchanged.

// Status "-bg" tints. v2 has no `status-*-bg` roles — alert and chip surfaces
// are a tint of the status colour over the ground. These alphas are the ones
// validate-tokens.mjs sweeps in TINT_LEVELS, which proves the status colour
// stays AA and body text AAA on the result in both modes. Changing them here
// without adding the new level to TINT_LEVELS ships an unverified surface.
const SHADCN_TINT_ALPHA = { light: 0.1, dark: 0.12, "mono-light": 0.1, "mono-dark": 0.12 };
...
function generateShadcnCSS() {
  ...
  // shadcn name → the v2 role it resolves to. Foregrounds come from
  // tokens.json#pairs wherever the fill declares one, so a pairing can never
  // drift from the contrast guarantee validate-tokens enforces.
  const pairFg = (surface) => tokens.pairs[surface].fg;
  const ROLES = [
    ["shadcn semantic", [
      ["background", "bg"],
      ["foreground", "text"],
      ["card", "surface-raised"],
      ["card-foreground", pairFg("surface-raised")],
      ["popover", "surface-raised"],
      ["popover-foreground", pairFg("surface-raised")],
      ["primary", "accent"],
      ["primary-foreground", pairFg("accent")],
      ["primary-hover", "accent-hover"],
      ["secondary", "surface"],
      ["secondary-foreground", pairFg("surface")],
      ["muted", "bg-subtle"],
      ["muted-foreground", "text-muted"],
      ["accent", "accent-subtle"],
      ["accent-foreground", "accent-hover"],
      ["destructive", "status-err"],
      // status-err declares no pair (it is a text colour first), so the label
      // on a solid destructive fill takes the ground — near-white on Sheet's
      // dark red, near-black on Field's light red. Pure white fails on Field.
    ...
    // Categorical series. Bronze leads; contour blue is the structural
    // second; ok/warn carry the third and fourth; border-strong is the
    // neutral tail. Anything wider than five series wants a real
    // categorical ramp — see the dataviz guidance before extending this.
    ["charts", [
      ["chart-1", "accent"],
      ["chart-2", "contour"],
      ["chart-3", "status-ok"],
      ["chart-4", "status-warn"],
      ["chart-5", "border-strong"],
    ]],
  ];

  const varBlock = (mode) => {
    const lines = [];
    for (const [label, entries] of ROLES) {
      lines.push(`  /* ${label} */`);
      for (const [name, role] of entries) {
        lines.push(`  --${name}: ${hexToHslTriplet(color(role, mode))};`);
      }
      if (label === "status") {
        lines.push(`  /* status tints — ${SHADCN_TINT_ALPHA[mode] * 100}% over bg, flattened */`);
        for (const role of ["status-err", "status-warn", "status-ok", "status-info"]) {
          const tint = blendHex(color(role, mode), color("bg", mode), SHADCN_TINT_ALPHA[mode]);
          lines.push(`  --${role}-bg: ${hexToHslTriplet(tint)};`);
        }
      }
    }
    lines.push(`  --scrim-alpha: ${MODES[mode].scrimAlpha};`);
    return lines.join("\n");
  };

  // The monochrome editions have no Tailwind dark-class analogue, so they ship
  // only under this system's own [data-theme] selectors.
  const monoBlocks = ["mono-light", "mono-dark"]
    .map((mode) => `/* ${MODES[mode].label} (${mode}) */\n${MODES[mode].dataTheme} {\n${varBlock(mode)}\n}`)
    .join("\n\n");

  return `/*
 * platforms/shadcn/tokens.css — GENERATED from tokens.json. Do not edit by hand.
 *
 * Jylhis Design System v2 "The Survey", mapped onto the shadcn/ui semantic
 * token names and emitted as bare HSL triplets for Tailwind, which consumes
 * them as hsl(var(--token) / <alpha-value>) so opacity modifiers keep working.
 *
 * Self-contained: pair it with fonts.css + fonts/ and import nothing else.
 * Sheet is :root; Field is both .dark and [data-theme="dark"]; the monochrome
 * Chalk / Graphite editions ship under [data-theme="mono-light|mono-dark"].
 *
 * See docs/INTEGRATION.md § Tailwind / shadcn.
 */
...
${varBlock("light")}
  ...
  /* Radii — the real scale. Consumers map Tailwind's borderRadius onto these
     directly (lg→md, md→sm, sm→xs) rather than shadcn's --radius minus-2px
     arithmetic, which bottoms out at 0 on a 4px canonical step. */
${Object.entries(tokens.radii).map(([k, v]) => `  --radius-${k}: ${v};`).join("\n")}

  /* Font families — the v2 three-role stack */
  --font-display: ${fontStack("display")};
  --font-body: ${fontStack("body")};
  --font-mono: ${fontStack("mono")};
  --font-heading: var(--font-display);
...
function generateGhostty(mode) {
  const { label, modus } = MODES[mode];
  const themeName = `jylhis-${MODES[mode].file}`;
  const accentKind = modus ? "the bronze accent" : "the ink accent (black/white)";
  ...
  const lines = [
    ...
    "#",
    `# ANSI 0\u20136 are ${modus ? `Modus ${modus} accents` : "graded grays"} so \`ls\`, \`bat\`, \`delta\`, \`git log\``,
    `# share the editor's palette. ANSI 11 is ${accentKind} \u2014 intentional`,
...
${ALL_MODES.map((m) => paletteStruct(m)).join("\n\n")}
...
function generateHyprland(mode) {
  const { label, file } = MODES[mode];

  const hex = (role) => color(role, mode).slice(1); // strip #

  if (isLight(mode)) {
    return `# Jylhis ${label} (${mode}) border colors for Hyprland
# GENERATED from tokens.json. Do not edit by hand.
...
misc {
    background_color = 0x${hex("bg")}            # ${file}
}
`;
  } else {
    return `# Jylhis ${label} (${mode}) border colors for Hyprland
...
function generateKvantum(mode) {
  const { label, kv: themeName } = MODES[mode];
...
<palette theme="${themeName}">
  ...
  <color role="Mid">${c("border")}</color>
  <color role="Dark">${c("text-muted")}</color>
  <color role="Shadow">${c("scrim")}</color>
...
function generateEmacsTheme(mode) {
  const { file: variant, polarity, label: groundName, modus } = MODES[mode];
  const themeName = `jylhis-${variant}`;
  const cap = polarity[0].toUpperCase() + polarity.slice(1);
  const desc = modus
    ? `Jylhis — ${polarity} theme. Bronze accent on the ${groundName} ground, Modus-derived syntax tuned for ${groundName}.`
    : `Jylhis — ${polarity} monochrome theme. Ink accent on the ${groundName} ground, grayscale syntax.`;
  const commentary = `${cap} "${variant}" variant`;
  const siblingMode = MODES[mode].sibling;
  const sibling = `jylhis-${MODES[siblingMode].file}-theme.el`;
  const otherTheme = `jylhis-${MODES[siblingMode].file}`;
  const otherMode = MODES[siblingMode].polarity;
  const label = polarity;
...
PALETTE is the value of \\='jylhis-sheet-palette', \\='jylhis-field-palette',
\\='jylhis-chalk-palette', or \\='jylhis-graphite-palette'
...
function generateRofi(mode) {
  const { file: label, label: themeLabel, polarity } = MODES[mode];
  const c = (role) => color(role, mode);
  const iconTheme = isLight(mode) ? "Papirus" : "Papirus-Dark";
  // accent-subtle / text-faint as opaque tokens (rofi has no rgba)
  const accentSubtle = c("accent-subtle");
  const textFaint = c("text-faint");

  const commentHeader = isLight(mode)
    ? `/* Jylhis Rofi \u2014 ${polarity} variant (${themeLabel}). GENERATED from tokens.json. */`
...
function generateWaybar(mode = "dark") {
  ...
  const { label, modus } = MODES[mode];
  const accentWord = modus ? "bronze accent" : "ink accent";

  return `/* Jylhis Waybar ${label} — GENERATED from tokens.json. Do not edit by hand.
 * ~/.config/waybar/style.css
 * ${label} background, ${accentWord}, monospace. Top bar.
 *
...
[urgency=critical]
background-color=${blendHex(c("status-err"), c("bg"), 0.12)}
text-color=${color("status-err", mode)}
border-color=${tokens.status["status-err"][mode]}          # status-err
default-timeout=0
...
function generateBase16(mode) {
  const slug = `jylhis-${MODES[mode].file}`;
  const label = `Jylhis ${MODES[mode].label}`;
...
#
#   imports = [ "\${pkgs.jylhis-themes}/share/jylhis/console/jylhis-${MODES[mode].file}.nix" ];
#
...
function parseInterfaces(dts) {
  ...
  while ((m = re.exec(dts))) {
    ...
    result.push({ name, extends: ext.replace(/\bextends\b/, "").trim(), props: parseProps(body) });
  }
  // Intersection type aliases (`export type X = A & B & { … };`) document the
  // same shape as an extending interface — TypeScript rejects `extends` when
  // the bases have non-identical members, so props sidecars use this form.
  const typeRe = /export type (\w+)\s*=\s*((?:[\w.<>[\]]+\s*&\s*)*)\{([\s\S]*?)\n\s*\};/g;
  while ((m = typeRe.exec(dts))) {
    ...
    const ext = bases
      ...
      .filter(Boolean)
      .join(" & ");
    result.push({ name, extends: ext, props: parseProps(body) });
  }
  return result;
}
...
function generateTokensMd() {
  ...
  for (const [role, v] of Object.entries(tokens.palette)) {
    ...
    L.push(`| \`${role}\` | ${groupOf(role)} | \`${v.light}\` | \`${v.dark}\` | \`${v["mono-light"]}\` | \`${v["mono-dark"]}\` | ${mdEscape(v.notes || "")} |`);
  }
  L.push(
    "",
  ...
  for (const role of ["text", "text-heading", "text-muted", "text-faint", "accent", "accent-hover", "brand", "contour"]) {
    L.push(`| \`${role}\` | ${ratio(role, "bg", "light")}:1 | ${ratio(role, "bg", "dark")}:1 | ${ratio(role, "bg", "mono-light")}:1 | ${ratio(role, "bg", "mono-dark")}:1 |`);
  }
  L.push(
    "",
  ...
  for (const [role, v] of Object.entries(tokens.syntax)) {
    L.push(`| \`${role}\` | ${mdEscape(v.modus || "")} | \`${v.light}\` | \`${v.dark}\` | \`${v["mono-light"]}\` | \`${v["mono-dark"]}\` |`);
  }
  L.push(
    "",
  ...
  for (const [role, v] of Object.entries(tokens.status)) {
    L.push(`| \`${role}\` | ${mdEscape(v.modus || "")} | \`${v.ansi || ""}\` | \`${v.light}\` | \`${v.dark}\` | \`${v["mono-light"]}\` | \`${v["mono-dark"]}\` |`);
  }
  L.push(
    "",
  ...
  tokens.ansi.forEach((a, i) => {
    L.push(`| ${i} | ${a.name} | \`${a.light}\` | \`${a.dark}\` | \`${a["mono-light"]}\` | \`${a["mono-dark"]}\` | ${mdEscape(a.role || "")} |`);
  });
  L.push(
    "",
  ...
  // ── 4b. Scaling ──
  const sc = ty.scaling;
  const readableStep = steps.indexOf(parseFloat(sc.readableFloor));
  const floorStep = steps.indexOf(parseFloat(sc.absoluteFloor));
  L.push(
    "### Scaling",
    "",
    ...
    "|---|---|---|---|",
    `| Readable | \`${sc.readableFloor}\` | \`--type-scale-${readableStep}\` | anything a user must read — prose, help text, labels, table cells, code |`,
    `| Absolute | \`${sc.absoluteFloor}\` | \`--type-scale-${floorStep}\` | glanceable chrome only — uppercase mono labels, badges, keycaps, refs |`,
    "",
    `Emitted as \`--type-readable-min\` / \`--type-floor\`. Relative (\`em\`) sizes below \`${sc.minRelativeEm}em\` must be floored with \`max(…, var(--type-floor))\` so nesting cannot compound below the hard minimum.`,
    "",
    `Fluid type: \`${sc.fluid}\` — the middle term must carry a \`rem\` component, or the size stops responding to the reader's font size between the bounds.`,
    "",
    `Targets: text resize to **${sc.targets.resizeText}** (WCAG 1.4.4), reflow at **${sc.targets.reflowWidth}** / **${sc.targets.reflowZoom}** zoom (1.4.10), and the 1.4.12 text-spacing set (line-height \`${sc.textSpacing.lineHeight}\`, letter-spacing \`${sc.textSpacing.letterSpacing}\`, word-spacing \`${sc.textSpacing.wordSpacing}\`, paragraph-spacing \`${sc.textSpacing.paragraphSpacing}\`).`,
    "",
...
out("platforms/shadcn/tokens.css", generateShadcnCSS());
...
// Monochrome editions (Chalk / Graphite) — every paired target the colour pair
// ships, graded to grayscale. GTK stays a single global file (no mono variant),
// as does glamour's colourless `notty`.
for (const mode of ["mono-light", "mono-dark"]) {
  const f = MODES[mode].file;
  out(`platforms/glamour/jylhis-${f}.json`, generateGlamour(mode));
  out(`platforms/ghostty/jylhis-${f}`, generateGhostty(mode));
  out(`platforms/hyprland/jylhis-${f}.conf`, generateHyprland(mode));
  out(`platforms/hyprlock/jylhis-${f}.conf`, generateHyprlock(mode));
  out(`platforms/kvantum/${MODES[mode].kv}.colors`, generateKvantum(mode));
  out(`platforms/emacs/jylhis-${f}-palette.el`, generateEmacsPalette(mode));
  out(`platforms/emacs/jylhis-${f}-theme.el`, generateEmacsTheme(mode));
  out(`platforms/rofi/jylhis-${f}.rasi`, generateRofi(mode));
  out(`platforms/waybar/style-${f}.css`, generateWaybar(mode));
  out(`platforms/mako/config-${f}`, generateMako(mode));
  out(`platforms/gimp/jylhis-${f}.gpl`, generateGimpPalette(mode));
  out(`platforms/adobe/jylhis-${f}.ase`, generateAdobeSwatch(mode));
  out(`platforms/base16/jylhis-${f}.yaml`, generateBase16(mode));
  out(`platforms/shell/fzf-${f}.sh`, generateFzf(mode));
  out(`platforms/bat/jylhis-${f}.tmTheme`, generateTmTheme(mode));
  out(`platforms/console/jylhis-${f}.nix`, generateConsole(mode));
  out(`platforms/plymouth/jylhis-${f}/jylhis.plymouth`, generatePlymouthManifest(mode));
  out(`platforms/plymouth/jylhis-${f}/jylhis.script`, generatePlymouthScript(mode));
  out(`platforms/hyperos/jylhis-${f}.mtz`, generateHyperOS(mode));
}
