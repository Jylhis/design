...
import { createFindings, report } from "./lib/findings.mjs";
...
const { findings, record: recordAt } = createFindings();
const record = (file, severity, msg) => recordAt(file, null, severity, msg);
...
// Match an actual import/require of a colour library, not the bare word — the
// design system has a theme literally named "chalk", which must not trip this.
const COLOR_LIB = /(?:from|import|require\()\s*\(?\s*["'](?:chalk|kleur|picocolors|ansi-colors)["']/;
...
function checkScript(rel) {
  ...
  //    on at least one error path. The shared findings reporter satisfies both
  //    the stderr and non-zero-exit contracts on the script's behalf.
  const usesSharedReport =
    /from\s+["']\.\/lib\/findings\.mjs["']/.test(src) && /\breport\s*\(/.test(src);
  const usesStderr = usesSharedReport ||
    /console\.error\s*\(/.test(src) || /process\.stderr\.write\s*\(/.test(src);
  const usesProcessExitNonZero = usesSharedReport || /process\.exit\s*\(\s*[1-9]/.test(src);
  if (!usesStderr) {
...
report("cli-conventions", findings, { checked: scripts.length, unit: "script" });
