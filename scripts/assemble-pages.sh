...
cp index.html tokens.css tokens-data.js styles.css fonts.css colors_and_type.css motion.css density.css _ds_bundle.js _site/
cp -r assets fonts preview components platforms docs _site/
cp -r scripts _site/
cp tokens.core.json DESIGN.md README.md SKILL.md _site/
cp -r themes _site/
