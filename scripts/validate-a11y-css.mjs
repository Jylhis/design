...
for (const f of files) {
  ...
  // Shadow doctrine: exactly one elevation shadow token, and only on floating
  // surfaces. In-flow surfaces separate by ground step + hairline (DESIGN.md
  // §Elevation). Inset rings are outlines drawn as shadows, not elevation.
  // Platforms (generated recolors) and tokens.css (the token definition) are out of scope.
  if (f !== 'tokens.css' && !f.startsWith('platforms/')) {
    for (const m of t.matchAll(/(?:^|[{;])\s*box-shadow\s*:\s*([^;}]+)/gm)) {
      const v = m[1].trim();
      if (v === 'none' || v.startsWith('inset') || v.includes('--shadow-float')) continue;
      err(f, `box-shadow "${v.slice(0, 40)}" is not the --shadow-float token (the one elevation shadow)`);
    }
  }
}
