;;; jylhis-catppuccin-mocha-theme.el --- Jylhis × Catppuccin Mocha (dark) -*- lexical-binding: t; -*-
;;
...
;;; Commentary:
;;
;;  Catppuccin "Mocha" recoloring of the Jylhis Emacs face map.
;;  Inspired by Rahul Juliato's Modus-Catppuccin write-up, this sibling reuses
;;  the same Modus-style semantic face targeting as `jylhis-roast-theme.el',
;;  but substitutes the palette block with values from the official Catppuccin
;;  Mocha flavor (https://catppuccin.com/palette/).  A sibling
;;  `jylhis-catppuccin-latte-theme.el' ships the light variant.
;;
;;  Unlike `jylhis-roast-theme.el', this file is hand-authored and does NOT
...
;;  Roast, so install/toggle behave the same way.
;;
;;  Install:
;;    (add-to-list 'custom-theme-load-path "~/.config/emacs/themes/")
;;    (load-theme 'jylhis-catppuccin-mocha t)
;;
;;  Pair with `jylhis-catppuccin-latte' via auto-dark:
...
(deftheme jylhis-catppuccin-mocha
  "Jylhis × Catppuccin Mocha — dark theme. Peach accent on Mocha base, Modus-style face map with Catppuccin syntax palette.")

(let ((class '((class color) (min-colors 256)))

      ;; ── Core palette (Catppuccin Mocha) ──
      (bg            "#1e1e2e") ; base
      (bg-subtle     "#181825") ; mantle
      (surface       "#11111b") ; crust
      (surface-raised "#313244") ; surface0
      (fg            "#cdd6f4") ; text
      (fg-muted      "#bac2de") ; subtext1
      (fg-heading    "#cdd6f4") ; text (Catppuccin text already very contrasty)
      (fg-faint      "#9399b2") ; overlay2
      (accent        "#fab387") ; peach — Jylhis copper analogue
      (accent-hover  "#eba0ac") ; maroon
      (accent-subtle "#3c2e22") ; dark peach wash (opaque approximation)
      (brand         "#fab387") ; peach
      (border        "#45475a") ; surface1
      (border-strong "#585b70") ; surface2
      (decorator     "#6c7086") ; overlay0

      ;; ── Syntax / semantic — Modus role taxonomy, painted with Mocha ──
      (syn-keyword   "#cba6f7") ; mauve
      (syn-string    "#a6e3a1") ; green
      (syn-number    "#74c7ec") ; sapphire
      (syn-function  "#89b4fa") ; blue (matches article's example)
      (syn-builtin   "#f5c2e7") ; pink
      (syn-type      "#94e2d5") ; teal
      (syn-variable  "#89dceb") ; sky
      (syn-tag       "#94e2d5") ; alias of type
      (syn-comment   "#7f849c") ; overlay1
      (syn-docstring "#8fbf86") ; muted green (manual)
  ...
  (custom-theme-set-faces
   ...
   ;; ─────────────────────────────────────────────
   ;; Terminal (vterm / ansi-term)  — Catppuccin Mocha ANSI
   ;; Bright-yellow is intentionally overridden to brand peach (Jylhis rule).
   ;; ─────────────────────────────────────────────
   `(ansi-color-black               ((,class :foreground "#1e1e2e" :background "#1e1e2e")))
   `(ansi-color-red                 ((,class :foreground "#f38ba8" :background "#f38ba8")))
   `(ansi-color-green               ((,class :foreground "#a6e3a1" :background "#a6e3a1")))
   `(ansi-color-yellow              ((,class :foreground "#f9e2af" :background "#f9e2af")))
   `(ansi-color-blue                ((,class :foreground "#89b4fa" :background "#89b4fa")))
   `(ansi-color-magenta             ((,class :foreground "#f5c2e7" :background "#f5c2e7")))
   `(ansi-color-cyan                ((,class :foreground "#94e2d5" :background "#94e2d5")))
   `(ansi-color-white               ((,class :foreground "#cdd6f4" :background "#cdd6f4")))
   `(ansi-color-bright-black        ((,class :foreground "#585b70" :background "#585b70")))
   `(ansi-color-bright-red          ((,class :foreground "#eba0ac" :background "#eba0ac")))
   `(ansi-color-bright-green        ((,class :foreground "#c2e89c" :background "#c2e89c")))
   `(ansi-color-bright-yellow       ((,class :foreground "#fab387" :background "#fab387")))
   `(ansi-color-bright-blue         ((,class :foreground "#b4befe" :background "#b4befe")))
   `(ansi-color-bright-magenta      ((,class :foreground "#cba6f7" :background "#cba6f7")))
   `(ansi-color-bright-cyan         ((,class :foreground "#89dceb" :background "#89dceb")))
   `(ansi-color-bright-white        ((,class :foreground "#f5e0dc" :background "#f5e0dc")))
...
(provide-theme 'jylhis-catppuccin-mocha)
...
;;; jylhis-catppuccin-mocha-theme.el ends here
