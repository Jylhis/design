...
```
platforms/ghostty/
├── jylhis-light           ← generated (light)
├── jylhis-dark            ← generated (dark)
└── config                 ← hand-authored (font, padding, opacity — not the palette)
```

The two theme files are generated from `tokens.core.json` +
`themes/jylhis.json`. The `config` file is a starting point you copy and
own — it deliberately doesn't ship a palette, only ergonomic defaults
(font, padding, opacity).

## Install

```bash
mkdir -p ~/.config/ghostty/themes
cp jylhis-light jylhis-dark ~/.config/ghostty/themes/
```
...
```
theme = light:jylhis-light,dark:jylhis-dark
```
...
Slot 11 (`bright-yellow`) is intentionally overridden to the bronze accent across every terminal target — Ghostty included. That's why prompts, directory permissions, and `ls --color` carry the Jylhis identity. Don't "fix" it.
