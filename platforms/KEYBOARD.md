...
- **AAA contrast** — `accent` is 8.35:1 on light, 8.64:1 on dark (both meet non-text AAA at ≥ 3:1, and our ring is treated as informational text-adjacent).
...
kbd {
  ...
  font-size: max(0.75em, var(--type-floor));
  padding: 0.1em 0.4em;
  background: var(--color-bg);
  border: 1px solid var(--color-border);
  border-bottom-width: 2px; /* key-cap depth — a thicker hairline, never a shadow */
  border-radius: 3px;
...
2. **Filter hints row.** `type a filter · tab to cycle · esc to close` in mono `text-muted` 0.8125rem.
...
- Backdrop: the scrim token `var(--color-scrim)` — `rgba(20,23,30,0.4)` in light,
  `rgba(5,6,10,0.55)` in dark. Click to dismiss.
...
Rule: if a modal, popover, overlay, or palette is open, the label `ESC` is visible in mono `text-muted` 0.8125rem, top-right of the modal. Always. Never behind a question mark or hover.
...
- [ ] Colors verified against `themes/jylhis.json` (ratios beside each claim).
