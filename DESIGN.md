...
- **THESIS.** The design system _is a topographic survey_: one datum
  ...
  empty; it is calm, cool, precise, and information-dense in an _ordered_ way.
...
- **STORY.** The visitor understands that a single measured source produces one
  identity everywhere, that its accessibility is _plotted, not asserted_, and
...
dark, night field-book). They are the _source and output_ of one survey. Extra
...
- Three type roles: Zilla Slab (plate titles), Hanken Grotesk (UI/body), IBM
  Plex Mono (data/code). The v1 mono-over-serif signature is retired.
- Flat: elevation is ground steps + 1px hairlines, never shadow.
- Unicode glyphs + thin `currentColor` survey marks are the icon set; the
  maker's mark is the prompt `jy ❯`, and the benchmark (△ in ◯) is the
...
- **Bronze** — `accent` (Sheet `#6f3e00` / Field `#e0a33a`): the only interactive
  colour. Links, focus rings, primary fills, the datum label, CTAs. AAA-tuned:
  ≥7:1 on every Sheet ground and on the Field ground.
- **Deep/Lit Bronze** — `accent-hover` (Sheet `#8a4d00` / Field `#f0b95c`):
  `:hover`/`:active` only, never a base colour; one bronze step lighter.
...
- **Warning** — `status-warn` (`#884900` / `#d9b34a`).
- **Success** — `status-ok` (`#006800` / `#6bbf6b`).
- **Info** — `status-info` (`#005e8b` / `#5fb8cf`).

### Modus (syntax) — code rendering only

Tuned to Emacs Modus Operandi (light) / Vivendi (dark) — current Modus 4.x
values, verbatim — so code is identical in the editor, on the web, in
`bat`/`delta`, and in Charm TUIs. Never repurposed for UI: `syn-keyword`
purple, `syn-string`/`syn-number` blue, `syn-function`/`syn-builtin` magenta,
`syn-type` teal, `syn-comment` grey; `syn-docstring` follows each edition's
own Modus mapping (green-faint on Sheet, cyan-faint on Field). Numbers borrow
the Modus `constant` slot (upstream leaves numbers unstyled). Exact values
live in `tokens.json` and are proven against the contrast validator.
...
- **Body** (Hanken Grotesk, 400, `1.0625rem`, 1.6): all prose. Measure capped at
  `72ch`.
- **Label** (IBM Plex Mono, 500, `0.85rem`, +0.1em, UPPERCASE): survey labels,
  coordinates, ticks, captions.
- **Code** (IBM Plex Mono, `max(0.85em, var(--type-floor))`, 1.6): inline code
  and code blocks.

The scale is ten fixed rem steps — `3.25 · 2 · 1.4 · 1.15 · 1.0625 · 0.95 · 0.9
· 0.875 · 0.85 · 0.8125` — emitted as `--type-scale-0…9`. Every step is a
multiple of the reader's root size; the system never sets `html { font-size }`.
`0.9rem` is the **readable floor** (anything a user must read), `0.8125rem` the
**absolute floor** (glanceable chrome only). See
[`docs/ACCESSIBILITY.md`](docs/ACCESSIBILITY.md#text-resizing--reflow).
...
- Small radius (`3px`), IBM Plex Mono at scale-6 (`0.85rem`), `0.5rem 1rem`
  padding.
- **Primary:** bronze `accent` fill, `bg` text, matching bronze border.
- **Ghost:** transparent, body-ink text, hairline border; hover shifts text +
  border to `accent`.
...
- **Tag:** mono at scale-7 on `bg-subtle`, hairline, `3px`; interactive tags shift
...
- **Card:** `surface-raised`, 1px hairline, `4px`, `md` padding; the title `<a>`
  inside takes the focus ring — never the card box itself.
...
- **Maker's mark — the prompt (`jy ❯`):** pure type in IBM Plex Mono, the
  chevron in benchmark vermilion; may carry the blinking caret on live
  surfaces. Appears once per surface. The benchmark (△ in ◯) stays on the
  survey plate as the datum symbol, drawn in thin `currentColor` stroke.
