...
- Accent: `#6f3e00` / `#e0a33a` — the only interactive color: links, focus
  rings, primary fills, ghost-button text
- Accent hover: `#8a4d00` / `#f0b95c` — hover/active twin, never a base color
- Brand: `#b5450e` / `#ef8a4a` — the benchmark vermilion. Use for _large_ marks
  where contrast isn't measured: the `jy ❯` maker's mark chevron, the datum
  triangle, sticker art. Never a link color (doesn't clear the AAA link floor
  on the Sheet ground), and never `status-err`
...
- Error `✗`: `#a60000` / `#f0685f`
- Warning `!`: `#884900` / `#d9b34a`
- OK `✓`: `#006800` / `#6bbf6b`
- Info `i`: `#005e8b` / `#5fb8cf`

**Syntax (Emacs Modus — Operandi light / Vivendi dark).** Canonical for _all_
...
3. **For non-web / programmatic artifacts** (pptx, image export, diagram tools)
   ...
   accent (`#6f3e00` / `#e0a33a`) to interactive chrome, the benchmark
...
- **Color application**: use the hex/RGB values above for direct application, or
  ...
  at 12% on the Sheet bg, 15% on Field) is `var(--color-accent-subtle)`, or
  `#e6e2dd` / `#262119` as an opaque approximation where rgba isn't available.
