...
function AIPanel({ withSelection = false, mode = 'panel' }) {
  return (
    ...
    shikiConfig: { theme: 'jylhis-light' },
})],`}</div>
              <p>why does shikiConfig.theme not pick up dark mode? it stays light even when prefers-color-scheme is dark.</p>
            </div>
          </div>
        )}
        <div className="ai-msg assistant">
          ...
          <div>
            ...
            <pre><span className="key">shikiConfig</span>: {`{`}{'\n'}  <span className="key">themes</span>: {`{`}{'\n'}    <span className="key">light</span>: <span className="str">'jylhis-light'</span>,{'\n'}    <span className="key">dark</span>: <span className="str">'jylhis-dark'</span>{'\n'}  {`}`}{'\n'}{`}`}</pre>
            <p>This generates two style sets gated on <code>[data-mode]</code> selectors, matching the theme's dark/light split.</p>
            <p><strong>2. Or, render twice</strong> server-side and toggle visibility — heavier payload but pixel-identical control. Given your jylhis-light / jylhis-dark palette already share token names, option 1 is cleaner.</p>
          </div>
        </div>
      </div>
