...
function App() {
  ...
  // stage scaling is handled by mocks/stage/stage.js (.mock-stage markup below)
  ...
  const runCommand = useCallback((raw) => {
    ...
    if (lower === '' ) { out = []; }
      ...
      out = [
        ...
        <span><span className="accent">  ▝▘▘▘▘▘▘▝▘ </span>  <span className="dim">theme</span>  jylhis · {theme === 'dark' ? 'Negative' : 'Print'}</span>,
        <span className="dim">  one accent (bronze) · monospace · no graphics.</span>,
      ];
    } else if (lower.startsWith('theme')) {
      const arg = lower.split(/\s+/)[1];
      const next = arg === 'dark' ? 'dark' : arg === 'light' ? 'light' : (theme === 'dark' ? 'light' : 'dark');
      setTweak('theme', next);
      out = [<span><span className="ok">✓</span> switched to <span className="accent">{next === 'dark' ? 'Negative' : 'Print'}</span> · every surface follows</span>];
  ...
  return (
    <div className="mock-stage mock-stage--vignette" data-canvas="1920x1080">
      <div className="mock-stage__frame">
        <div
          className={`mock-stage__canvas screen ${tweaks.density === 'compact' ? 'compact' : ''}`}
        ...
        >
          ...
          <div className="desktop" onClick={() => setOpenMenu(null)}>
            <div className={`mock-tui-wall ${tweaks.wallpaper}`} />
