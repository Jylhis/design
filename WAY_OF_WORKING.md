...
_what_ the system is, while this document says _how_ to change it.
...
4. Open a PR. Write a Conventional Commits-style title. Describe _why_
   the change exists, not just _what_ it does.
...
6. **Merge to `main`.** Tag the merge commit:
   ...
   ```
7. **The showcase ships via Cloudflare** from the monorepo. The `_site`
   artifact is assembled by `scripts/assemble-pages.sh`; there is no
   Pages workflow in this repo. Confirm the live showcase shows the new
   version badge.
...
- A second accent color, a third theme, an icon font.
