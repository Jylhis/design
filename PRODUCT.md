...
- **One cool light, one cool dark, both first-class and both AAA-body.** Not a
  ...
  _source and output_ of one survey, not tinted twins.
...
- **Validator contract (the spec of "done"):** token schema + WCAG contrast +
  ...
  PR; never silence it.
- **Components:** a React library of 20 components, each shipping `<Name>.jsx`
  with `.d.ts` and a `card.html` specimen, styled by
  `components/components.css` using tokens only, one class per component.
...
- **Hard constraints (from ENGINEERING_PRINCIPLES.md, treat as invariant):**
  ...
  `prefers-reduced-motion` honored; no icon font/SVG sprite/emoji, Unicode
  glyphs and thin `currentColor` survey marks instead; no hex duplication;
  dual-theme (Sheet/Field) parity is mandatory. (v2 note: the former
  no-sans-serif rule is retired — the type system is a three-role stack; see
  Brand Commitments.)
- **Deliberately small, slow-changing.** Two themes, one accent, one type
  triad, a fixed list of targets. Adding a target or token group is an argued decision.
...
- **Palette (Modus-lean, cool, tritanopia-checked):** two editions —
  **Sheet** (light, ground `#f6f8fb`) and **Field** (dark, ground `#0d0f14`);
  ...
  (Sheet `#6f3e00` / Field `#e0a33a`); **Modus blue** structural contour
...
- **Maker's mark:** the prompt — **`jy ❯`**, pure type set in IBM Plex Mono,
  the chevron in benchmark vermilion. The benchmark glyph (△ in ◯) remains the
  survey-plate datum symbol, a motif rather than the mark; `assets/favicon.svg`
  renders the type mark.
...
- Live production site: https://jylhis.com (Astro, served via Cloudflare).
- Deployed showcase served via Cloudflare from the monorepo (`index.html` +
  preview/prototype assets, assembled by `scripts/assemble-pages.sh`).
...
3. **Accessibility is a mechanical contract.** AAA body, AA meta, decorative
   ...
   contour _is_ the contrast threshold; a role below its floor does not print.
...
- **v1.0 shipped.** `1.0.0` was tagged 2026-07-25; the goal now is holding the
  contract stable. Avoid churn that needlessly breaks downstream consumer pins.
